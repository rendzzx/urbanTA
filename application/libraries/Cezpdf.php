...
include_once(BASEPATH . '../application/libraries/Cpdf.php');
...
// 
// version 009 (versioning is linked to Cpdf.php)
//
...
// ------------------------------------------------------------------------------
	...
	function __construct($params = array('paper' => 'a4', 'orientation' => 'portrait'))
	{
		extract($params);
...
// ------------------------------------------------------------------------------
	...
	function ezStartPageNumbers($x, $y, $size, $pos = 'left', $pattern = '{PAGENUM} of {TOTALPAGENUM}', $num = '')
	{
		...
		$this->ez['pageNumbering'][$i][$this->ezPageCount] = array('x'      => $x,
		                                                           'y'      => $y,
		                                                           'pos'    => $pos,
		                                                           'pattern'=> $pattern,
		                                                           'num'    => $num,
	...
	function ezPrvtTableColumnHeadings($cols, $pos, $maxWidth, $height, $decender, $gap, $size, &$y, $optionsAll = array())
	{
		...
		while ($ok == 0)
		{
			foreach ($cols as $colName=> $colHeading)
			{
				...
				$this->ezText($colHeading, $size, array('aleft'        => $pos[$colName],
