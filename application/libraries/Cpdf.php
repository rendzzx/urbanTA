...
class Cpdf
{
	 ...
	 */
	var $currentColour = array('r'=> - 1,
	                           'g'=> - 1,
	                           'b'=> - 1);
	/**
	 * current colour for stroke operations (lines etc.)
	 */
	var $currentStrokeColour = array('r'=> - 1,
	                                 'g'=> - 1,
	                                 'b'=> - 1);
	/**
	...
	function o_destination($id, $action, $options = '')
	{
		...
		{
			case 'new':
				$this->objects[$id] = array('t'   => 'destination',
				                            'info'=> array());
				$tmp = '';
				switch ($options['type'])
				{
	...
	function o_viewerPreferences($id, $action, $options = '')
	{
		...
		{
			case 'new':
				$this->objects[$id] = array('t'   => 'viewerPreferences',
				                            'info'=> array());
				break;
			case 'add':
	...
	function o_catalog($id, $action, $options = '')
	{
		...
		{
			case 'new':
				$this->objects[$id] = array('t'   => 'catalog',
				                            'info'=> array());
				$this->catalogId = $id;
				break;
			case 'outlines':
	...
	function o_pages($id, $action, $options = '')
	{
		...
		switch ($action)
		{
			...
			case 'mediaBox':
				...
				break;
			case 'font':
				$o['info']['fonts'][] = array('objNum' => $options['objNum'],
				                              'fontNum'=> $options['fontNum']);
				break;
			case 'xObject':
				$o['info']['xObjects'][] = array('objNum'=> $options['objNum'],
				                                 'label' => $options['label']);
				break;
			case 'out':
	...
	function o_outlines($id, $action, $options = '')
	{
		...
		{
			case 'new':
				$this->objects[$id] = array('t'   => 'outlines',
	...
	function o_fontDescriptor($id, $action, $options = '')
	{
		...
		{
			case 'new':
				$this->objects[$id] = array('t'   => 'fontDescriptor',
				                            'info'=> $options);
				break;
			case 'out':
				$res = "\n" . $id . " 0 obj\n<< /Type /FontDescriptor\n";
		...
		switch ($action)
		{
			case 'new':
				...
				$this->objects[$id] = array('t'   => 'info',
				                            'info'=> array('Creator'     => 'R and OS php pdf writer, http://www.ros.co.nz',
				                                           'CreationDate'=> $date));
				break;
			case 'Title':
			case 'Author':
	...
	function o_annotation($id, $action, $options = '')
	{
		...
		switch ($action)
		{
			case 'new':
				...
				{
					case 'link':
						$this->objects[$id] = array('t'   => 'annotation',
						                            'info'=> $options);
						$this->numObj ++;
					...
					case 'ilink':
						...
						$this->objects[$id] = array('t'   => 'annotation',
						                            'info'=> $options);
						$this->numObj ++;
						$this->o_action($this->numObj, 'new', array('type' => 'ilink',
						                                            'label'=> $label));
						$this->objects[$id]['info']['actionId'] = $this->numObj;
						break;
				}
	...
	function o_page($id, $action, $options = '')
	{
		...
		switch ($action)
		{
			case 'new':
				$this->numPages ++;
				$this->objects[$id] = array('t'   => 'page',
				                            'info'=> array('parent' => $this->currentNode,
	...
	function o_contents($id, $action, $options = '')
	{
		...
		{
			case 'new':
				$this->objects[$id] = array('t'   => 'contents',
				                            'c'   => '',
				                            'info'=> array());
	...
	function o_image($id, $action, $options = '')
	{
		...
		switch ($action)
		{
			case 'new':
				...
				$this->o_pages($this->currentNode, 'xObject', array('label' => $options['label'],
				                                                    'objNum'=> $id));
	...
	function o_encryption($id, $action, $options = '')
	{
		...
		switch ($action)
		{
			case 'new':
				// make the new object
				$this->objects[$id] = array('t'   => 'encryption',
				                            'info'=> $options);
				$this->arc4_objnum = $id;
	...
	function setEncryption($userPass = '', $ownerPass = '', $pc = array())
	{
		...
		if ($this->arc4_objnum == 0)
		{
			...
			$this->o_encryption($this->numObj, 'new', array('user' => $userPass,
			                                                'owner'=> $ownerPass,
			                                                'p'    => $p));
		}
	}
	...
	function setStrokeColor($r, $g, $b, $force = 0)
	{
		...
		{
			$this->objects[$this->currentContents]['c'] .= "\n" . sprintf('%.3f', $r) . ' ' . sprintf('%.3f', $g) . ' ' . sprintf('%.3f', $b) . ' RG';
			$this->currentStrokeColour = array('r'=> $r,
			                                   'g'=> $g,
			                                   'b'=> $b);
		}
	}
	...
	function setFontFamily($family, $options = '')
	{
		if (! is_array($options))
		{
			if ($family == 'init')
			{
				...
				$this->fontFamilies['Helvetica.afm'] = array('b' => 'Helvetica-Bold.afm',
				                                             'i' => 'Helvetica-Oblique.afm',
				                                             'bi'=> 'Helvetica-BoldOblique.afm',
				                                             'ib'=> 'Helvetica-BoldOblique.afm');
				$this->fontFamilies['Courier.afm'] = array('b' => 'Courier-Bold.afm',
				                                           'i' => 'Courier-Oblique.afm',
				                                           'bi'=> 'Courier-BoldOblique.afm',
				                                           'ib'=> 'Courier-BoldOblique.afm');
				$this->fontFamilies['Times-Roman.afm'] = array('b' => 'Times-Bold.afm',
				                                               'i' => 'Times-Italic.afm',
