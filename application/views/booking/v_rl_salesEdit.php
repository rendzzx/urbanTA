  ...
  <script type="text/javascript" src="<?=base_url('lainnya/dist/js/jquery.mask.min.js')?>"></script>
   ...
   <script type="text/javascript">
    ...
    $('#disc').change(function(){
      ...
      price2 = price - parseFloat(disc_amt);

      if(discno=='') //(discno==''||planprice=='') 
      {
        var result = 0;
        var net_price = price2 - result;
        $("#txt_aditional_disc").val(formatNumber(result));
      }
      else
      {
        var result = (parseInt(discno) * parseInt(price2)) / 100 ;
        var net_price = price2 - result;
        $("#txt_aditional_disc").val(formatNumber(result));
      }
      ...
      $('#txt_discount2').val(formatNumber(disc));
    });
