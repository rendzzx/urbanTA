...
<div class="content-wrapper">
  ...
  <div id="loader" class="loader" hidden="true"></div> 
    <div class="form-group">
      ...
      <!-- <div class="tittle-top pull-right"><b><?php if($status=='N'){echo 'ADD ROI Entry '.$phase->descs;}else{echo 'EDIT ROI Entry Edit '.$phase->descs;} ?></b></div> -->
      <div class="tittle-top pull-right">
      <b>
        <?php if($status=='R')
                { if($Type=='P'){echo 'Revision ROI Entry Prioritas';} else{echo 'Revision ROI Entry Regular';}}
              else
                {if($Type=='P'){echo 'EDIT ROI Entry Prioritas';} else{echo 'EDIT ROI Entry Regular';}} ?>
        
      </b></div>
    </div>        
  </div><br>
  <form role="form" class="form-horizontal" enctype="multipart/form-data" id="form_nup" method ="POST" >
  <?php if($status == 'R' || $status == 'P'){?>
    <div class="row border-bottom white-bg dashboard-header">    
      <div class="row">
      <FONT style="color:rgb(188, 1, 8);">
        <div class="form-group">
          <label class="col-sm-3 control-label">Alasan / Reason :</label>
          <div class="col-sm-7">
            <!-- <select class="select2_demo_1 form-control"  name="reason_cd" id="reason_cd" data-placeholder="Select Reason"><?php echo $reason ?></select> -->
            <label class="control-label" id="lblreason" name="lblreason"></label>
          </div>
        </div>
        <div class="form-group">
          <label class="col-sm-3 control-label">Remarks :</label>                
          <div class="col-sm-7">
            <!-- <textarea class="form-control" placeholder="Input Remarks" name="remarkReason" id="remarkReason" style=" height: 50px;"></textarea> -->
            <label class="control-label" id="lblremarks" name="lblremarks"></label>
          </div>
        </div>
        </FONT>
      </div>           
      ...
      <div class="row">
        <div class="col-xs-12">        
            <div class="ibox-content">
              ...
              <!-- <input type="hidden" name="prospective1" id="prospective1"> -->
            </div>
            <?php } ?> 
             
              <!-- <input type="hidden" name="prospective" id="prospective"> -->
              ...
              <div class="form-group">
                ...
                <div class="col-sm-3">
                  <select class="select2_demo_1 form-control" name="country_cd" id="country_cd" data-placeholder="Select Country">
                  </select>
                </div>
                <div class="col-sm-4">
              ...
              <div class="form-group">
                ...
                <div class="col-sm-7">
                  <select class="select2_demo_1 form-control col-sm-2" name="nationality" id="nationality" data-placeholder="Select Nationality">
              ...
              <div class="form-group" hidden="hidden">
                ...
                <div class="col-sm-3" >
                  ...
                  <select class="select2_demo_1 form-control" name="nuptype" id="nuptype" data-placeholder="Select ROI Type">
  ...
  function Loaddata(){
    ...
    if(status !='N'){
      ...
      $.getJSON(site_url, function (data) {                
                ...
                // if (data[0].status == 'P' || data[0].status == 'E'){
                //   $('#prospective').val(data[0].business_id).trigger('change');                  
                // } else {
                //   $('#prospective').val(data[0].business_id);
                // }
                ...
                if(data[0].nup_qty == Roiqty){
                  $('#Roiamt').val(formatNumber(data[0].total_nup_amt));  
                }else{
                  $('#Roiamt').val(formatNumber(data[0].nup_amt * Roiqty));
                }

                $('#lblreason').text(data[0].reason_descs);
                $('#lblremarks').text(data[0].remarks);
...
$('#submit').click(function(){
  ...
  var seqno =$('#seqno').val();  
  var screen_type = '<?php echo $screen_type ?>';
  ...
  var Roiqty = $('#Roiqty').val();
  var Roiamt = $('#Roiamt').val();
  var reason_cd = $('#reason_cd').val();
  var remarkReason = $('#remarkReason').val();
  // var pro = $('#prospective').val();
  // var pro1 = $('#prospective1').val();
  // var bussID = '';
  var site_url = "<?php echo base_url('c_nup_cfld/savenup')?>";
  var dataform = $('#form_nup').serializeArray();
  // var bussID = $('#bussiness_id').val();
  // alert(bussID);
  // return;
  ...
  if($('#form_nup').valid()){
    ...
    function(isConfirm){
      if (isConfirm) {
        ...
        var site_url2 = '<?php echo base_url("c_nup_cfld/check_attachment")?>';
        $.post(site_url2,
...
$('#change_unit').click(function(){
  ...
  // return;
  // var Header_ID = '<?php echo $HeaderID ?>';
  var Type = '<?php echo $Type ?>';
  var statusEdit = '<?php echo $status ?>';
  var screen_type = '<?php echo $screen_type ?>';
  ...
  $.ajax({
    ...
    success: function(data, status) {     
    if(screen_type=='M'){
      var site_url = '<?php echo base_url("c_mobile_cfld/update_session_mobile")?>';
    $.post(site_url,
        {
          id:myBookId,
          lot_descs:lot_descs,
      ...
      window.location.href = "<?php echo base_url('c_nup_landed_cfld/indexEdit')?>"+"/"+statusEdit+"/"+Type;
    }         
