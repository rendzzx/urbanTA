        ...
        <div class="form-group">
            ...
            <select data-placeholder="Choose a Level No" class="select2 form-control" id="level_no" name="level_no" required>
                ...
                <?php foreach ($level_no as $key) { ?>
                    <option value="<?php echo $key->level_no?>">Level <?php echo $key->level_no ?></option>
                <?php }  ?>
            </select>
        </div>
        <div class="form-group">
            <label for="level_descs">Level Description</label>
            <input type="text" id="level_descs" class="form-control" placeholder="Level Description" name="level_descs" readonly>
        </div>
        <div class="form-group">
...
$(document).ready(function(){
    ...
    $('#level_no').change(function(){
        block(true);
        var id = $(this).val()
        $.getJSON("<?php echo base_url('C_Setting_Mu/getByIDmeterreading/');?>"+id, function (data) {
            $('#level_descs').val(data[0].descs)
            block(false);
        });
        $.getJSON("<?php echo base_url('C_Setting_Mu/getByIDmeterreading_metertype');?>", function (data) {
    ...
    $('#savefrm').click(function(){
        ...
        $.ajax({
            url : "<?php echo base_url('C_Setting_Mu/save_meterreading');?>",
            type:"POST",
            data:datafrm,
            dataType:"json",
