...
<form id ="frmEditor" class="form-horizontal" method="post" action="<?php echo site_url(); ?>c_lottype/save_lottype" enctype="multipart/form-data">
    <div class="box-body">
        <div class="form-group">
            <label for="groupcd">Property</label>
            <select data-placeholder="Choose a Group" class="select2 form-control" id="property_cd" name="property_cd" style="width: 100% !important">
                <option value=""></option>
                <?php if(!empty($property)){foreach ($property as $key) { ?>
                    <option value="<?php echo $key->property_cd?>"><?php echo $key->descs ?> </option>
                <?php }}  ?>
            </select>
        </div>
        <div class="form-group">
...
// ------------------------ end function loading
        ...
        var id = $('#modal').data('id');
            ...
            if (id > 0) {
                ...
                $.getJSON("<?php echo base_url('c_lottyped/getByID');?>" + "/" + id, function (data) {
                    console.log(data)
                    $('#property_cd').val(data[0].property_cd).trigger("change");
    ...
    $(document).ready(function(){
        ...
        $(document).on('hidden.bs.modal', '#modal', function () {
            ...
            $("#savefrm_lotd").prop('disabled', false)
        });
        ...
        $('#userfile').bind('change', function() {
            ...
            if (size>=max_size) {
                ...
                $("#savefrm_lotd").prop('disabled', true)
                $('#picturebox').attr('src', '');
            }
            else{
                $("#savefrm_lotd").prop('disabled', false)
      ...
      highlight: function (element, errorClass, validClass) {
        ...
        $('#savefrm_lotd').click(function(event){
