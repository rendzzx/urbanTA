  ...
  <div class="middle-box text-center loginscreen animated fadeInDown">
      ...
      <div style="text-align:left !important;" class="">
        ...
        <form id="frm1" method="post" role="form" class="form-horizontal" enctype="multipart/form-data">
          ...
          <div class="form-group">
             <label for="subjectnewsfeed" class="col-xs-3">Name</label>
             <div class="col-xs-9">
          ...
          <div class="form-group">
             <label for="subjectnewsfeed" class="col-xs-3">Email</label>
             <div class="col-xs-9">
          ...
          <div class="form-group">
             <label for="subjectnewsfeed" class="col-xs-3">No Telp</label>
             <div class="col-xs-9">
               <input type="text" class="form-control" name="notelp" id="notelp" placeholder="notelp">
             </div>
          </div>
          <input type="hidden" id="sosmed" name="sosmed" value="<?php echo $sosmed; ?>"></input>
         ...
         <div class="pull-right">
          ...
          <script>
            ...
            $(document).ready(function () {
              ...
              var sosmed = $('#sosmed').val();
              var sos = ""
              if (sosmed == "FB") {
                var sos = localStorage.getItem('facebook');
                var sos = JSON.parse(sos);
                $('#id').val(sos.id);
                $('#name').val(sos.name);
                $('#email').val(sos.email);
              }
              else{
                var sos = localStorage.getItem('google');
                var sos = JSON.parse(sos);
                $('#id').val(sos.id);
                $('#name').val(sos.displayName);
                $('#email').val(sos.emails[0].value);
              }
   ...
   $('#submit').click(function(){
      ...
      if($('#frm1').valid())
      {
        ...
        var site_url = "<?php echo base_url('userStaff/SignUpGuest')?>";
        $.ajax({
          url: site_url,
          type: "POST",
          data: dataform,
