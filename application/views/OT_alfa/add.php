 ...
 .tabs-container > .nav > li > a {
   ...
   <!--          <div class="form-group">
            ...
            <div class="form-group">
               <label >Total</label>
               <div class="col-sm-12">
                  <input id="trx_amt_show" name="trx_amt_show" class="form-control " type="text" disabled="true">
                  <input id="trx_amt" name="trx_amt" class="form-control " type="hidden" disabled="true">
               </div>
            </div>
            <div class="form-group">
  ...
  <script type="text/javascript">
    ...
    function hitung_trxamt(){
      ...
      if(rate==0||rate==null||isNaN(rate)){
        ...
        $('#trx_amt').val(total.toFixed(2));
        $('#trx_amt_show').val(formatNumber(total.toFixed(2)));
...
// }
   ...
   $('.select2').select2({
    ...
    //   var over_cd = data[2]
    //   block(true);
    //   $.getJSON("<?php echo base_url('c_overtime_a/zoom_ot_type');?>" + "/" + over_cd, function (data) {
    ...
    $( "#over_cd" ).change(function(e) {
      ...
      if(over_cd!=='') {
            var site_url = '<?php echo base_url("c_overtime_a/zoom_zone_type")?>';
            $.post(site_url,
              {over_cd:over_cd},
    ...
    // console.log(this.options[this.selectedIndex].dataset);
      $('#floor').val("");
      if(debtor_acct!=='') {
            var site_url = '<?php echo base_url("c_overtime_a/zoom_lot_no")?>';
            $.post(site_url,
    ...
    $('#savefrm').click(function(event){
      ...
      if($('#form_nup').valid()){
        ...
        // document.getElementById('loader').hidden=false; 
            $.ajax({
                url : "<?php echo base_url('c_overtime_a/save');?>",
                type:"POST",
                data: datafrm,
                dataType:"json",
    ...
    function parseDate(input) {
     ...
     // $('#modal').one('shown.bs.modal', function (e) {
      function LoadData(){
        ...
        if (id > 0) {
            $.getJSON("<?php echo base_url('c_overtime_a/getByID');?>" + "/" + id, function (data) {
              console.log(data)
