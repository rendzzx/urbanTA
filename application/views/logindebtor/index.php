...
var tblDebtor,tblstaff;
$(function(){
$('.select2').select2();

   // tblDebtor = $('#tblDebtor').DataTable( 
   //  {
   ...
   //      "serverSide": true,
   //      "ajax":{
   //          "url":"<?php echo base_url('c_logindebtor/getTable');?>",  
 ...
 $("div.toolbar1").html('<div class="input-group"><b>Search : <div class="input-group"><input type="text" style="width: 150px; height: 25px; border-bottom: 1px;" id="txt_search_debtor" name="txt_search_debtor" ><a class="btn blue-bg btn-sm" onclick="fn_search_debtor()"><i class="fa fa-search"></i></a></div></div><br><br></b>');
    ...
    // });
     $("#txt_search_debtor").keyup(function(event){
        var a = $('#txt_search_debtor').val();
        if(a==''){
            tblDebtor.ajax.reload(null,true);   
        }
     ...
     tblstaff = $('#tblstaff').DataTable( 
    {
        ...
        "serverSide": true,
        "ajax":{
            "url":"<?php echo base_url('c_logindebtor/getTableStaff');?>",  
            "data":{"sSearch": function(d){
...
function SetCheckBox(val) {
    var rows = tblDebtor.rows().indexes();
    for (var i = 0; i < rows.length ; i++) {
        var menuId = tblDebtor.rows(i).data()[0].rowID;
        $('#tblDebtor' + ' input[name=cb_' + menuId + ']').prop('checked', val);
    }
...
function SetCheckBox2(val) {
 ...
 function email(debtor_acct,project,entity){
        var debtor_acct = debtor_acct;
        var rows = tblDebtor.rows('.selected').indexes();      
                                    ...
                                    var btnYes = $('<input/>')
                                        .attr({
                                            ...
                                            onclick: 'Approval(\''+debtor_acct+'\',\''+project+'\',\''+entity+'\')',
                                            value: 'Yes'
                                        });
                                    ...
                                    $('#modal').data('debtor_acct', debtor_acct).modal('show');
    ...
    function Approval(debtor_acct,project,entity){
    // swal(debtor_acct);
    // return;
     document.getElementById('loader').hidden=false;
      $.ajax({
                    url : "<?php echo base_url('c_logindebtor/Approval');?>",
                    type:"POST",
                    data: { debtor_acct: debtor_acct, project: project, entity: entity},
                    dataType:"json",
   ...
   function Approval2(name,project,entity){
    ...
    // return;
     ...
     document.getElementById('loader').hidden=false;
      $.ajax({
                    url : "<?php echo base_url('c_logindebtor/Approval2');?>",
                    type:"POST",
   ...
   function proses(){
    // alert('aa');
    var ids = tblDebtor.rows().indexes();
            var ACCESS_CODE = '';
            var selData = [];
            for(var i = 0; i < ids.length; i++)
            {
                var menuID = tblDebtor.rows(i).data()[0].rowID;
                var debtor_acct = tblDebtor.rows(i).data()[0].debtor_acct;
                var entity = tblDebtor.rows(i).data()[0].entity_cd;
                var project = tblDebtor.rows(i).data()[0].project_no;
                var chx = $('#tblDebtor input[name=cb_'+menuID+']').prop('checked');
                ...
                if(ACCESS_CODE != '')
                {
                    ...
                    sysMenuGroup.debtor_acct = debtor_acct;
            ...
            $.ajax({
                url: '<?php echo base_url("c_logindebtor/save");?>',
                data: {models: selData},
                method: 'post',
                dataType: 'json',
            })
   ...
   function proses2(){
    ...
    var ids = tblstaff.rows().indexes();
            ...
            console.log(ids.length);
            $.ajax({
                url: '<?php echo base_url("c_logindebtor/savestaff");?>',
                data: {models: selData},
                method: 'post',
                dataType: 'json',
            })
    ...
    <div class="wrapper wrapper-content" >
        ...
        <div class="tabs-container">
                        <ul class="navii navii-tabs">
                            ...
                            <!-- <li class=""><a data-toggle="tab" href="#tab-2"> Debtor</a></li> -->
                        </ul>
                                        ...
                                        <div class="ibox-content">
                                            <div class="table-responsive">                       
                                                ...
                                                <button id="proses2" class="btn biru-bg" onclick="proses()"><i class="fa fa-envelope fa-fw"></i> <span class="hidden-xs">Process</span></button>
                                                    <table id="tblDebtor" class="table table-striped table-bordered table-hover dataTables" cellspacing="0" width="100%">
                                                        <thead>    
                                                            <th>No.</th> 
                                                            <th>Action</th>   
