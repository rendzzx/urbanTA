...
<form id ="frmEditor" class="form-horizontal" method="post" action="<?php echo site_url(); ?>c_lottype/save_lottype" enctype="multipart/form-data">
    <div class="box-body">
        ...
        <div class="form-group">
            ...
            <div class="col-xs-8">
                <input type="text" class="form-control" name="remarks" id="remarks" placeholder="Gallery Title">
            </div>
        </div>
        <div class="form-group">
            <label for="groupcd">Unit Group</label>
            <select data-placeholder="Choose a Group" class="select2 form-control" id="unitgroup" name="unitgroup" style="width: 100% !important">
                <option value=""></option>
                <?php if(!empty($unitgroup)){foreach ($unitgroup as $key) { ?>
                    <option value="<?php echo $key->zone_cd?>"><?php echo $key->descs ?> </option>
                <?php }}  ?>
            </select>
        </div>
        <div class="form-group">
...
// ------------------------ end function loading
    ...
    function loaddata(){
        var id = $('#modal').data('id');
            ...
            if (id > 0) {
                ...
                $.getJSON("<?php echo base_url('c_lottype/getByID');?>" + "/" + id, function (data) {
                    console.log(data);
                    var spec_info = data[0].spec_info
                    $('#specinfo').summernote({
                        height: 300,
                    });
                    $('#specinfo').summernote('code',spec_info)

                    $('#remarks').val(data[0].remarks);
                    $('#unitgroup').val(data[0].zone_cd).trigger('change');
    ...
    $(document).ready(function(){
        $(document).on('hidden.bs.modal', '#modal', function () {
            ...
            $("#savefrm_tbllot").prop('disabled', false)
        });
        $(".select2").select2();
        ...
        $('#userfile').bind('change', function() {
            ...
            if (size>=max_size) {
                ...
                $("#savefrm_tbllot").prop('disabled', true)
                $('#picturebox').attr('src', '');
            }
            else{
                $("#savefrm_tbllot").prop('disabled', false)
      ...
      highlight: function (element, errorClass, validClass) {
        ...
        $('#userfile').fileupload({
            url: "<?php echo base_url('c_lottype/save_lottype');?>",
            dataType: 'json',
            add: function (e, data) {
                jqXHRData = data
                isFile = true;                
        ...
        $('#savefrm_tbllot').click(function(){
              block(true);
            // document.getElementById('loader').hidden=false;
            if($('#frmEditor').valid()){
                var id = $('#modal').data('id');
                var datafrm = $('#frmEditor').serializeArray();
                var specinfo = $('#specinfo').summernote('code')
                datafrm.push({name:"specinfo",value:specinfo},{name:"isFile",value:isFile},{name:"id",value:id})
                var obj = new Object();
                // obj.id = id;
                obj.isFile = isFile;
                if(isFile){
