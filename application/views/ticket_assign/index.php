...
<link rel="stylesheet" type="text/css" href="<?=base_url('app-assets/vendors/js/gallery/photo-swipe/photoswipe.css')?>">
<link rel="stylesheet" type="text/css" href="<?=base_url('app-assets/vendors/js/gallery/photo-swipe/default-skin/default-skin.css')?>">
  ...
  <div class="content-wrapper">
     ...
     <div class="content-header row">
        <div class="content-header-left col-md-4 col-12 mb-2">
           <br><br>
           <h3 class="content-header-title">Ticket Assign</h3>
        </div>
...
<script src="<?=base_url('app-assets/vendors/js/gallery/masonry/masonry.pkgd.min.js')?>" type="text/javascript"></script>
<script src="<?=base_url('app-assets/vendors/js/gallery/photo-swipe/photoswipe.min.js')?>" type="text/javascript"></script>
<script src="<?=base_url('app-assets/vendors/js/gallery/photo-swipe/photoswipe-ui-default.min.js')?>" type="text/javascript"></script>
<script src="<?=base_url('app-assets/js/scripts/gallery/photo-swipe/photoswipe-script.js')?>" type="text/javascript"></script>
...
<script type="text/javascript">
  ...
  var table = $('#table').DataTable({
    "ordering": false,
    "ajax" : {
        "url" : "<?php echo base_url('C_Ticket_Assign/gettable');?>",
        "type": "POST"
    },
    "columns": [
  ...
  function format ( d ) {
    ...
    if (d.seq_no_ticke==="") {
      ...
      $.getJSON("<?php echo base_url('C_Ticket_Update/getpict');?>" + "/" + d.seq_no_ticket, function (data) {
        if (data.length>0) {
          ...
          if (pict==null || pict=='') {
            ...
            $('#img'+d.complain_no).append(
            '<div class="carousel-item active">'+
              '<img class="img-thumbnail img-fluid w-100" src="'+data[0].file_url+'" itemprop="thumbnail" alt="Image description" style="cursor: pointer;" onclick=modal("'+d.seq_no_ticket+'") />'+
            '</div>'
            )
          }
        }
    ...
    '<div class="card box-shadow-0 border-info">'+
      '<div class="card-content collapse show">'+
        '<div class="card-body">'+
          ...
          '<div class="row">'+
            '<div class="col-sm-12">'+
              '<button onclick="assign('+d.complain_no+')" type="button" class="btn btn-glow btn-bg-gradient-x-blue-cyan btn-block">Assign to PIC</button>'+
            '</div>'+
          '</div>'+
        '</div>'+
      '</div>'+
  ...
  function assign(id){
    ...
    $('#modaltitle').html('Assign');
    $('#modalbody').load("<?php echo base_url("C_Ticket_Assign/assign/")?>"+id);
    $('#modal').data('id', 0);
    $('#modal').modal('show');
    $('.modal-footer').remove()
  }
  ...
  function loadfilter(){
      $.getJSON("<?php echo base_url('C_Ticket_Assign/getfilter');?>", function (data) {
