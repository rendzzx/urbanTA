									...
									<div class="form-body">
				                        ...
				                        <div class="form-group row">
					                        	<div class="col-md-12">
						                        	...
						                        	<div class="col-md-4">
						                        		<div class="i-checks">
					                                          <label  id="radioR"> <input type="radio" name="refund_type" id="ref" value="Y" checked> <i></i> Refund </label> &emsp;
			                			...
			                			<div class="form-group row">
				                         ...
				                         <div class="form-group row">
								        	...
								        	<!-- <div class="col-2">
									            <div class="form-group">
									            	<label for="file-upload" class="custom-file-upload"><i class="ft-plus"></i>
													</label>
													<input id="file-upload" type="file" name="pict5" id="pict5" />
									            	<p style="margin-left: 25px">Picture 5</p>
									            	<div class="i-checks">
			                                       		<input type="radio" name="main_pic" id="pic5" value="5"> 
			                                        </div>
...
<script>
	$(document).ready(function(){
	    ...
	    CKEDITOR.replace('terms_descs', {
	      height: '350px',
	      // extraPlugins: 'forms',
	        removeButtons:'Save,Form,Checkbox,Radio,TextField,Textarea,Select,Button,ImageButton,HiddenField'
	    });
		var nup_type = '<?php echo $nup_type ?>';
		if(nup_type=='' || nup_type==null){
			// alert('a')
		}else{
			// alert(nup_type)
			$('#nuptype').val(nup_type);
	    ...
	    $('#nuptype').on('change',changeHandler);
	    $('#start').datepicker({
	    format: 'dd/mm/yyyy',
	      autoclose: true,
	...
	function changeHandler(){
	    	...
	    	var nuptype = $('#nuptype').find(':selected').val();
             ...
             $.getJSON("<?php echo base_url('c_nup_online/getbyNUPtype');?>" + "/" + nuptype, function (data) {
				...
				//   if(data.data1[0].badges == 'G'){
				// 	  $('#btn-badge-one').val(data.data1[0].badges);
				//   }
		          if(data.data1[0].refund_type == 'Y'){
		          	$('#ref').iCheck('check');
		          }else{
		          	$('#nonref').iCheck('check');
		          }
		          if(data.data1_adm.length<1){
		          	...
		          	CKEDITOR.instances.info_nup.setData('', function()
				    {
				        this.checkDirty();  // true
				    });
				    CKEDITOR.instances.terms_descs.setData('', function()
				    {
				        this.checkDirty();  // true
				    });
		          }else{
		          	...
		          	CKEDITOR.instances.info_nup.setData(data.data1_adm[0].info_nup, function()
				    {
				        this.checkDirty();  // true
				    });
				    CKEDITOR.instances.terms_descs.setData(data.data1_adm[0].terms_descs, function()
				    {
				        this.checkDirty();  // true
				    });
		          }
