...
<div class="content-wrapper">
  ...
  <div id="loader" name="loader" class="loader" hidden="true"></div> 
    <div class="form-group">
      <div class="tittle-top pull-left">            
      <?php echo $project; ?><br>
        <?php echo($agent->agent_name)?>
      </div>
      <div class="tittle-top pull-right"><?php if($status=='N'){ if($Type=='P'){echo 'ADD ROI Entry Prioritas';}else{echo 'ADD ROI Entry Regular';}}else{if($Type=='P'){echo 'Edit ROI Entry Prioritas';}else{echo 'Edit ROI Entry Regular';}} ?></div>
    </div>        
  </div>
        ...
        <form role="form" class="form-horizontal" enctype="multipart/form-data" id="form_nup" method ="POST" >
          <div class="ibox-content">
            ...
            <div class="form-group">
              ...
              <div class="col-sm-4">
                <input type="text" class="form-control nomer" name="HP" id="HP" data-inputmask="'mask':'999999999999'" placeholder="8xxxxxxxxxx" onkeyup="validateInp(this);">
            ...
            <div class="form-group">
              <label class="col-sm-3 control-label" name="lblnoktp" id="lblnoktp">ID No.<!-- <FONT COLOR="RED">*</FONT> --></label>
              ...
              <div class="col-sm-7">
                <input type="text" class="form-control nomer" name="noktp" id="noktp" placeholder="Input ID Number">
              </div>
            </div>
            <div class="form-group">
              <label class="col-sm-3 control-label">Alamat / Address<FONT COLOR="RED">*</FONT></label>                
              <div class="col-sm-7">
                <textarea class="form-control" placeholder="Input Address" name="address" id="address" style=" height: 50px;text-transform:uppercase"></textarea>
              </div>
            </div>
            <div class="form-group">
          ...
          <div class="box-footer pull-right" style="">
          
            <input type="button" name="submit" id="submit" value="Save" style="margin-left: 10px;" class="btn btn-primary pull-right">
...
// alert(rowID);
              ...
              var sn = $('#seqno').val();
                ...
                $('#modalTitle').html('<b>Add File</b>');
                // alert(rowID);
                $('div.modal-body').load("<?php echo base_url('c_nup_mobile_cfld/addNew');?>"); //+"/"+descs+"/"+rowID);
                // $('#modal').data('descs', descs);
  ...
  if($('#form_nup').valid()){
    ...
    function(isConfirm) {
      if (isConfirm) {
        ...
        var site_url2 = '<?php echo base_url("c_nup_mobile_cfld/check_attachment_roi_mobile")?>';
        $.post(site_url2,
                ...
                success:function(data, status){
                  ...
                  if(data.status != 'Failed'){
                    $.ajax({
                      ...
                      type:"POST",
                      data: { id:id,status:status_nup,HeaderID:headerID,Type:type_roi},
                      dataType:"json",
  ...
  function(isConfirm){
    if (isConfirm) {
      ...
      if(status == 'E'){
        ...
        var seqno =$('#seqno').val();      
        var site_url = '<?php echo base_url("c_nup_mobile_cfld/check_delete_attachment") ?>';
        $.post(site_url,
          {seqno:seqno},
          function(data,status){
            ...
            $.ajax({
              ...
              success: function(data, status){
                var Cluster_cd = '<?php echo $Cluster_cd ?>';
                var Type = '<?php echo $Type ?>';                                
                var url = '<?php echo base_url("c_mobile_cfld/ClusterLanded")?>'+'/'+Cluster_cd+'/'+Type;
...
$('#change_unit').click(function(){
  ...
  $.ajax({
    ...
    dataType: "json",
    success: function(data, status) {              
      window.location.href = "<?php echo base_url('c_mobile_cfld/index')?>";
    },
    error: function(jqXHR, textStatus, errorThrown) {
      swal(textStatus + ' Save : ' + errorThrown);
    }
