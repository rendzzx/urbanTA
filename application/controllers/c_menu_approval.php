...
class c_menu_approval extends Core_Controller
{
    ...
    public function index()
    {
    	...
    	$this->load_content_top_menu('menu_approval/index',$content);
    }

      public function zoom_parentid(){  
//         $table = 'sys_menu_approval(nolock)';
...
//         $comboParentID = $this->m_wsbangun->getCombo($table, $object);
    ...
    public function addnew($id=""){
        ...
        $this->load->view('menu_approval/add', $content);
    ...
    public function getByID($MenuID=''){
        ...
        $data = $this->m_wsbangun->getData_by_criteria_adm('sys_menu_approval',$where);
    ...
    public function delete(){
        ...
        $data = $this->m_wsbangun->deletedataweb('sys_menu_approval',$where);
    ...
    public function getTable()
    {
        ...
        $aColumns  = array('row_number', 'MenuID','Title', 'URL', 'ParentMenuID', 'Icon','OrderSeq');
        // $aColumns = array('entity_cd', 'entity_name');
        $sTable = 'mgr.sys_menu_approval';
    ...
    public function save_menu(){
            ...
            if ($_POST) 
            {
                ...
                if(empty($parent_id)){
                    ...
                    // var_dump($criteria);
                    if($menu_id>0) {
                        $data = $this->m_wsbangun->updateDataweb('sys_menu_approval',$data, $criteria);
                        // var_dump($data);exit();
                        if($data !="OK"){
                            $msg= $data;
                            $st = 'Fail';
                     ...
                     //   $this->m_user_log->insert(add_user_log("newsfeed Name " . $newsfeed_name, $this->m_users->get_by_uname($this->session->userdata('uname')), $this->m_activities->get_by_title("ADD newsfeed")));
                        ...
                        $data = $this->m_wsbangun->insertDataweb('sys_menu_approval',$data);
                        if($data !="OK"){
                            $msg= $data;
                            $st = 'Fail';
                        }else{
