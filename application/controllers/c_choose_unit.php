...
class c_choose_unit extends Core_Controller {
    ...
    // 			'project_no'=>$project,
    	...
    	$this->load_content_top_menu('unit2/unitindex2',$ContentAllData);
    	// $this->load->view('unit2/unitindex',$ContentAllData);
    }
    ...
    public function indexNew()
    {
    	...
    	$this->load_content_top_menu('unit2/unitindex2',$ContentAllData);
    	// $this->load->view('unit2/unitindex2',$ContentAllData);
    }
    ...
    public function goto_form($status=''){
        ...
        $cons = $this->session->userdata('Tscons');
        $nuptype = $this->m_wsbangun->getCombo_cons($cons,$table,$crit);
	...
	public function show_edit_data($ID=''){
		...
		$where =array('entity_cd'=>$entity,
					...
					'rowID'=>$ID);
		$cons = $this->session->userdata('Tscons');
		$data = $this->m_wsbangun->getData_by_criteria_cons('v_nup_update',$cons,$where);
  ...
  //       $prjName = empty($dtPrj) ? '' : $dtPrj[0]->descs;
   ...
   //      	var_dump($rsvdate);	
	...
	public function getTable()
    {
        ...
        //untuk PK diharap diletakan di awal array
        $aField = array('id', 'subject', 'content','status');
        $aColumns  = array('rowID', 'nup_no', 'Handphone', 'Email', 'nup_type', 'reserve_date','STATUS','product_type');
...
// var_dump($Search);
        ...
        // $rResult = $DB2->get($sTable);
        // $rResult = $DB2->query($sql_data);
        $param =" Where STATUS='A' AND  entity_cd='".$entity."' AND project_no= '".$project."' ".$filter_search. " ";
        $cons = $this->session->userdata('Tscons');
        $rResult = $this->m_wsbangun->getlisttablecus_cons($cons,$sTable,(int)($iDisplayStart),(int)($iDisplayLength),$SordField,$SortdOrder,$param);
	...
	public function getImage()
	{
        ...
        $cons = $this->session->userdata('Tscons');
        $dtAtch = $this->m_wsbangun->getData_by_query_cons($cons,$sql);
        // var_dump($dtAtch);
        // var_dump($dtAtch);
