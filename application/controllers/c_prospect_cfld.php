...
class C_prospect_cfld extends Core_Controller {
	...
	public function index()
    {
		...
		$scrennResolution = $this->input->post('getResolution', TRUE);
		// var_dump($scrennResolution);	
    ...
    // 			'project_no'=>$project,
    	...
    	$this->load_content_top_menu('prospect/index',$ContentAllData);
    }
    ...
    // 			'project_no'=>$project,
    	...
    	$this->load_content_top_menu('nup/NupIndexCfldNew',$ContentAllData);
    }
    ...
    public function goto_form($status=''){
    	...
    	$sql = "IF NOT EXISTS(SELECT Header_sequence_no FROM mgr.rl_nup_attachment WHERE Header_sequence_no=$seqno) ";
	...
	public function check_attachment_roi($seqno='',$from='',$status=''){
		

		if($seqno==''){
			$seqno =  $this->input->post('seqno', TRUE);
			$from =  $this->input->post('from', TRUE);
			$status = $this->input->post('statusRoi', TRUE);
  ...
  //       $cnt = $this->check_attachment($seqno,'IN');
	...
	public function chosen_city_(){
        ...
        $city = $this->m_wsbangun->getData_by_query($sql);
        if(!empty($city)){
	...
	public function show_edit_data($ID=''){
		...
		$data = $this->m_wsbangun->getData_by_criteria('rl_prospect',$where);
	...
	public function show_e_data($ID=''){
		...
		$where =array('entity_cd'=>$entity,
					'project_no'=>$project,
					'rowID'=>$ID);
		$dataa = $this->m_wsbangun->getData_by_criteria('rl_reserve_nup_hd',$where);
	...
	public function zoom_nuptype(){
      ...
      public function edit_rev_mobile($status='',$rowID='',$type='')
	{
		// $rowID = (string)$this->input->post('ID', TRUE);
        ...
        $Cluster_cd = $this->session->userdata('Cluster_cd');
        ...
        foreach ($data as $value) {
		    ...
		    $process = $this->m_wsbangun->getData_by_query($queryLotDesc);
...
// exit;
   ...
   //      	);
			...
			// var_dump($dtProduct);
			$this->load_content_top_menu('booking_mobile_cfld/v_rl_nupNew_cfld',$content);
		}
		else{
			// var_dump('edit');
			$content = array('comboTnup'=>'',
...
// var_dump($dtProduct);
	...
	public function edit_rev($status='',$rowID='', $header_id='',$type='',$screen_type='')
	{
		// $rowID = (string)$this->input->post('ID', TRUE);
  		// $status = (string)$this->input->post('status', TRUE);
  		// var_dump($status);SubmitStatus
  		// var_dump($rowID);
  		// var_dump($header_id);
        $entity = $this->session->userdata('Tsentity');
        $project = $this->session->userdata('Tsproject');
		...
		if($lot_descs!=''){
			$abc = strrpos($lot_descs, ",");
        	$lot_descs = substr($lot_descs,0,$abc);	
		}
...
// exit;
   ...
   //      	);
			...
			// var_dump($dtProduct);
			$this->load_content_top_menu('booking_cfld/v_rl_nupNew_cfld_edit',$content);
		} else {
			// var_dump($status);
			// var_dump('edit');
			$content = array('comboTnup'=>'',
...
// var_dump($dtProduct);
  ...
  //       $prjName = empty($dtPrj) ? '' : $dtPrj[0]->descs;
	...
	public function insert_mobile($status='',$Type='') 
	{
		// $status = (string)$this->input->post('status', TRUE);
		// var_dump($status);
		// var_dump($unit_book);
        ...
        $crit = array('nup_type', 'descs');
        $nuptype = $this->m_wsbangun->getCombo3($table,$crit);
     ...
     //    $my_array = array("stuff1", "stuff2", "stuff3");
        ...
        $table = 'cf_country (nolock)';
        $crit = array('country_code', 'descs');
        $cbcountry = $this->m_wsbangun->getCombo4($table,$crit);

        $table = 'cf_nationality (nolock)';
        $crit = array('nationality_cd', 'descs');
        $cbnationality = $this->m_wsbangun->getCombo4($table,$crit);

        $table = 'v_rl_nup_type (nolock)';
        $crit = array('nup_type', 'descs');
        $cbroitype = $this->m_wsbangun->getCombo($table,$crit);
	...
	public function insert($status='',$rowID='0') 
	{
		// $status = (string)$this->input->post('status', TRUE);
		// var_dump($status);
		// var_dump($rowID);
        ...
        // $table = 'cf_media(nolock) order by nup_type ASC';
        // $crit = array('nup_type', 'descs');
        // $nuptype = $this->m_wsbangun->getCombo3($table,$crit);

        $table = 'cf_media(nolock)';
        $value = array('media_cd', 'descs');
        $media = $this->m_wsbangun->getCombo($table,$value);

        $table = 'rl_prospect_category(nolock)';
        $value = array('category_cd', 'descs');
        $category = $this->m_wsbangun->getCombo($table,$value);

        $table = 'rl_prospect_type(nolock)';
        $value = array('type_cd', 'descs');
        $type = $this->m_wsbangun->getCombo($table,$value);

        $table = 'rl_prospect_reason(nolock)';
        $value = array('reason_cd', 'descs');
        $reason = $this->m_wsbangun->getCombo($table,$value);

        $table = 'rl_prospect_age(nolock)';
        $value = array('age_cd', 'descs');
        $age = $this->m_wsbangun->getCombo($table,$value);

        $table = 'rl_prospect_income(nolock)';
        $value = array('income_cd', 'descs');
        $income = $this->m_wsbangun->getCombo($table,$value);

        $table = 'rl_prospect_reason_nb(nolock)';
        $value = array('reason_nb_cd', 'descs');
        $reason_nb = $this->m_wsbangun->getCombo($table,$value);
        ...
        $content = array('comboMedia'=>$media,
        ...
        $this->load_content_top_menu('prospect/v_prospect',$content);
	}
  ...
  //       return $cnt;
	...
	public function save_prospect()
	{
		if($_POST)
		{
			...
			// if(empty($bussiness_id)|| $bussiness_id==0){

			// }
			$salutation =  strtoupper($this->input->post('salutation', TRUE));
        	$customer = strtoupper($this->input->post('customer', TRUE));
        	$media_cd = strtoupper($this->input->post('media', TRUE));
        	$media_detail = strtoupper($this->input->post('media_detail', TRUE));
        	$city = strtoupper($this->input->post('city', TRUE));
        	$email = strtoupper($this->input->post('Email', TRUE));
        	$category_cd = strtoupper($this->input->post('category', TRUE));
        	$category_remarks = strtoupper($this->input->post('remarks', TRUE));
        	$unit_qty = $this->input->post('unit_qty', TRUE);
        	$buy_unit_reason = strtoupper($this->input->post('buy_unit_reason', TRUE));
        	$buy_lavon_reason = strtoupper($this->input->post('buy_lavon_reason', TRUE));
        	$payment_method = strtoupper($this->input->post('payment_method', TRUE));
        	$age = strtoupper($this->input->post('age', TRUE));
        	$marital_status = strtoupper($this->input->post('marital_status', TRUE));
        	$occupation = strtoupper($this->input->post('occupation', TRUE));
        	$monthly_income = strtoupper($this->input->post('monthly_income', TRUE));
        	...
        	$city = strtoupper($this->input->post('city', TRUE));
        	$reason_nb = strtoupper($this->input->post('nb_lavon_reason', TRUE));
        	// $customer_type = strtoupper($this->input->post('customer_type', TRUE));
		        ...
		        if($status=='N'){
		        	...
		        	$data = array(
		        		...
		        		'class_cd'=>$class_cd,
		        		'media_cd'=>$media_cd,
		        		'prospect_media_detail'=>$media_detail,
		        		'prospect_city'=>$city,
		        		'email_addr'=>$email,
		        		'handphone'=>$HP,
		        		'prospect_category_cd'=>$category_cd,
		        		'prospect_qty_property'=>$unit_qty,
		        		'prospect_type'=>$buy_unit_reason,
		        		'prospect_reason'=>$buy_lavon_reason,
		        		'prospect_payment'=>$payment_method,
		        		'prospect_age'=>$age,
		        		'marital_status'=>$marital_status,
		        		'prospect_occupation'=>$occupation,
		        		'income_cd'=>$monthly_income,	        		
		        		'audit_user'=>$user,
		        		'audit_date'=>$today,
		        		'remarks'=>$category_remarks,
		        		'reason_nb_cd'=>$reason_nb
		        		,'entry_date'=>$today
		        		,'staff_in_charge'=>$user
		        		// ,
		        		// 'customer_type'=>$customer_type        		
		        		
		        		);	  

		        	$data_cf_business = array(
		        		'business_id'=>$Number,
		        		'salutation'=>$salutation,
		        		'category'=>'I',
		        		'class_cd'=>$class_cd,
			        	...
			        	$sql1 = "SELECT count(*) as cnt from mgr.rl_prospect where email_addr='$email' or handphone='$HP'";
			        	$cek_data = $this->m_wsbangun->getData_by_query($sql1);
			        	$count = $cek_data[0]->cnt;
			        	if($count > 0)
			        	{
			        		$a = "There's already another costumer using that Email Address or Handphone number";
			                $psn = "Failed";
			        	} else {
			        		$insert = $this->m_wsbangun->insertData($table, $data);
			        		if($insert == 'OK')
	                        {
	                        	$insert_cf_business =  $this->m_wsbangun->insertData('cf_business', $data_cf_business);
	                        	if($insert_cf_business=='OK'){
	                        		$a="Data has been saved successfully";
	                            	$psn = "OK";	
	                        	}else{
	                        		// $where=array('business_id'=>$Number);
	                        		// $this->m_wsbangun->deletedata($table,$where);
	                        		$a= $insert_cf_business;
	                            	$psn = "Failed";	
	                        	}
		        		...
		        		$where_cf_business=array('business_id'=>$bussiness_id);
		        		$where = array('rowID'=>$rowID);
						$update = $this->m_wsbangun->updateData($table,$data, $where);

						if($update == 'OK')
	                        {
	                        	$update_cf_business = $this->m_wsbangun->updateData('cf_business',$data_cf_business, $where_cf_business);
	                        	if($update_cf_business=='OK'){
	                        		
									$a = "Data has been Updated successfully";								
		                            $psn = "OK";
	                        	}else{
	                        		$a= $update_cf_business;
	                            	$psn = "Failed";
	                        	}
 ...
 //    }
    ...
    public function SubmitStatus(){
       ...
       //          $profile_mail = $data[0]->email_profile;
	    		if($Type=='P'||$Type=='ROP '){
	    				$sql1 = "exec mgr.x_rl_submit_roi_prioritas '".$headerID."'";
	    		}else{
	...
	public function addNew()//($dt=null, $row=null)
    {
        ...
        //     return;
        // }
        $this->load->view('booking_cfld/v_nup_upload_cfld');
    }
	...
	public function getTable($product_cd='')
    {
       ...
       $entity = $this->session->userdata('Tsentity');
        ...
        $aColumns  = array('row_number','rowID');
        // $aColumns = array('entity_cd', 'entity_name');
        $sTable = 'mgr.v_prospect';
...
// var_dump($Search);
        ...
        $param =" Where audit_user ='$name' AND rowID > 0 ".$filter_search;
...
// var_dump($Search);
        ...
        // $rResult = $DB2->get($sTable);
        // $rResult = $DB2->query($sql_data);
        $param =" Where (status not in ('A','V', 'S','C','E','X') or (status = 'S' and old_status in ('R','N'))) and userid='".$name."' AND entity_cd='".$entity."' AND project_no= '".$project."' ".$filter_search;
        // $param =" Where status = 'N' and userid='".$name."' AND entity_cd='".$entity."' AND project_no= '".$project."' ".$filter_search;
        $rResult = $this->m_wsbangun->getlisttablenup($sTable,(int)($iDisplayStart),(int)($iDisplayLength),$SordField,$SortdOrder,$param);
