        ...
        public function showland($lotno = null)
    {
        ...
        public function validasiNew(){
            ...
            if($_POST){
               ...
               $Nupno = $this->input->post('Nupno',TRUE);
                ...
                //                     $updateCntr = $this->m_wsbangun->setData_by_query("UPDATE mgr.pm_lot set nup_counter = ISNULL(nup_counter,0) - 1 where lot_no = '$value' and entity_Cd = '$entity' and project_no = '$project'");
                ...
                //update - 1 counter where lotno in reserve_dt
                $sql = "Update mgr.pm_lot set nup_counter = ISNULL(nup_counter,0) - 1 where entity_Cd = '$entity' and project_no = '$project' ";
                $sql .= " and lot_no IN (select a.lot_no from mgr.rl_reserve_nup_dt a(nolock) where a.entity_cd='$entity' and a.project_no = '$project' and nup_no='$Nupno') and nup_counter > 0";
                $updateCntr = $this->m_wsbangun->setData_by_query_cons($cons,$sql);
                if ($updateCntr != 'OK'){
                    $msg1=array('Pesan'=>$updateCntr,
                                     'nup'=>$NupNo,
                                     'notif'=>'GAGAL');
                ...
                foreach ($data as $val) {                                    
                    $lotCount = "SELECT COUNT (*) as cnt FROM mgr.pm_lot_web (NOLOCK) WHERE entity_Cd = '$entity' and project_no = '$project' and lot_no ='$val' and ISNULL(nup_counter,0) > 10000";
                ...
                $blnc = (int)$LotQty - (int)$TotQty;
                // var_dump($LotQty);
                // var_dump($TotQty);
                // var_dump('expression');
                // var_dump($blnc);
                // var_dump($TotCount);
                if((int)$TotCount <= (int)$LotQty){
                    ...
                    $sql = "Update mgr.pm_lot set nup_counter = ISNULL(nup_counter,0) + 1 where entity_Cd = '$entity' and project_no = '$project' ";
                    $sql .= " and lot_no IN (select a.lot_no from mgr.rl_reserve_nup_dt a(nolock) where a.entity_cd='$entity' and a.project_no = '$project' and nup_no='$NupNo')";
                    $updateCntr = $this->m_wsbangun->setData_by_query_cons($cons,$sql);
                    if ($updateCntr != 'OK'){
                    $msg1=array('Pesan'=>$updateCntr,
                                     'nup'=>$NupNo,
        ...
        public function validasi2(){
            ...
            if($_POST){
                ...
                $sql = "Update mgr.pm_lot set nup_counter = ISNULL(nup_counter,0) - 1 where entity_Cd = '$entity' and project_no = '$project' ";
                $sql .= " and lot_no IN (select a.lot_no from mgr.rl_reserve_nup_dt a(nolock) where a.entity_cd='$entity' and a.project_no = '$project' and nup_no='$Nupno') and nup_counter > 0";
                $updateCntr = $this->m_wsbangun->setData_by_query($sql);
                if ($updateCntr != 'OK'){
                    $msg1=array('Pesan'=>$updateCntr,
                                     'nup'=>$Nupno,
                                     'notif'=>'GAGAL');
