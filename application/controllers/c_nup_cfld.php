...
class C_Nup_cfld extends Core_Controller {
    ...
    // 			'project_no'=>$project,
    	...
    	$this->load_content_top_menu('nup/NupIndexCfld',$ContentAllData);
    }
	...
	public function check_attachment($seqno='',$from=''){
		...
		$sql = "SELECT count(*) as cnt , count(file_url) as counter FROM mgr.rl_nup_hd_attachment (nolock) ";
  ...
  //       $cnt = $this->check_attachment($seqno,'IN');
		...
		// 			'file_attached'=>$isnull,
		// 			);

        if($cnt == 0){
        	// if($cnt==0 ){
				// $this->m_wsbangun->deletedata2('rl_nup_hd_attachment',$where);
			// }
	...
	public function show_edit_data($ID=''){
		...
		$where =array('entity_cd'=>$entity,
					...
					'rowID'=>$ID);
		$cons = $this->session->userdata('Tscons');
		$data = $this->m_wsbangun->getData_by_criteria_cons($cons,'v_nup_update_hd',$where);
	...
	public function show_edit_data_dt($ID=''){
		// $rowID = (string)$this->input->post('ID', TRUE);
		// $seqno = (string)$this->input->post('seqno', TRUE);
		$entity = $this->session->userdata('Tsentity');
        $project = $this->session->userdata('Tsproject');
		...
		$where =array('entity_cd'=>$entity,
					...
					'rowID'=>$ID);
		$cons = $this->session->userdata('Tscons');
		$data = $this->m_wsbangun->getData_by_criteria_cons('v_nup_update_dt',$cons,$where);


		echo json_encode($data);

	}
	public function get_pros_dt($ID=''){
	
		$where =array(
					'business_id'=>$ID);
		$cons = $this->session->userdata('Tscons');
		$data = $this->m_wsbangun->getData_by_criteria_cons($cons,'v_prospect_dt',$where);
...
// var_dump($dtProduct);
	...
	public function edit_rev($status='',$rowID='', $header_id='',$type='',$screen_type='')
	{
        ...
        $cons = $this->session->userdata('Tscons');
        $cbcountry = $this->m_wsbangun->getCombo_cons($cons,$table,$crit);
        // var_dump($cbcountry);exit();
        $dtProduct='';

        $user = $this->session->userdata('Tsuname');
        $table = 'v_prospect_dt';
        $crit = array('business_id', 'name','hand_phone');
        $where = array('audit_user'=>$user);
        $cons = $this->session->userdata('Tscons');
        $cbprospect = $this->m_wsbangun->getCombo2_cons($cons,$table,$crit,$where);
...
// var_dump($dtProduct);
  ...
  //       $prjName = empty($dtPrj) ? '' : $dtPrj[0]->descs;
		...
		if($status == 'A' || $status == 'P' || $status == 'V' || $status == 'R'){	
			...
			$this->load_content_top_menu('booking_cfld/v_rl_nupRev2',$content);
			...
			$this->load_content_top_menu('booking_cfld/v_rl_nupNew',$content);	
			
		}
        
	}

	public function edit_rev_a_($status='',$rowID='')
	{
		// $rowID = (string)$this->input->post('ID', TRUE);
  		// $status = (string)$this->input->post('status', TRUE);
        $entity = $this->session->userdata('Tsentity');
  ...
  //       $prjName = empty($dtPrj) ? '' : $dtPrj[0]->descs;
   ...
   //      }
     ...
     //    $my_array = array("stuff1", "stuff2", "stuff3");
        ...
        $crit = array('country_code', 'descs');
        $cons = $this->session->userdata('Tscons');
        $cbcountry = $this->m_wsbangun->getCombo4_cons($cons,$table,$crit);

        $table = 'cf_nationality (nolock)';
        $crit = array('nationality_cd', 'descs');
        $cons = $this->session->userdata('Tscons');
        $cbnationality = $this->m_wsbangun->getCombo4_cons($cons,$table,$crit);
	...
	public function insert($status='',$type='') 
	{
		// $status = (string)$this->input->post('status', TRUE);
		// var_dump($status);
		// var_dump($unit_book);
...
//         exit();
  ...
  //       return $cnt;
	...
	public function save_cf_business($ID='',$name='',$hp='',$email='',$user='',$country_cd='',$address='',$noktp='',$city='',$npwp='',$salutation='', $nationality='',$customer_type=''){
		$cons = $this->session->userdata('Tscons');
		$table = 'cf_business';      
	...
	public function savenup()
	{
		if($_POST)
		{
		   ...
		   //      	var_dump($rsvdate);	
        	...
        	// $prospect = $this->input->post('prospective', TRUE);
        	// $prospect1 = $this->input->post('prospective1', TRUE);
        	// if(empty($prospect) || $prospect == NULL){
        	// 	$pro = $prospect1;
        	// }else{
        	// 	$pro = $prospect;
        	// }
	        ...
	        $data_cf_business = $this->save_cf_business($bussiness_id,$customer,$HP,$Email,$user,$country_cd,$address,$noktp,$city,$npwp,$salutation,$nationality,$customer_type);
	        ...
	        if($status=='N'){
	        	$data = array('entity_cd'=>$entity,
		        	...
		        	// if($countDt == 0){
                        ...
                        $sql = "select HeaderID, Header_sequence_no, STATUS, business_id from mgr.rl_reserve_nup_hd where Header_sequence_no='".$seqno."'";
                        $cons = $this->session->userdata('Tscons');
                        $dtSub = $this->m_wsbangun->getData_by_query_cons($cons,$sql);
                        $headerID = $dtSub[0]->HeaderID;
                        $status_nup = $dtSub[0]->STATUS;
                        
                        $uBussId = $dtSub[0]->business_id;

                        $whereAttc = array('Header_sequence_no'=>$seqno);
                        $tblAtt = 'rl_nup_hd_attachment';
                        $dataAtt  = array('business_id' => $uBussId, );

                        $updateAtt = $this->m_wsbangun->updateData2($tblAtt,$dataAtt, $whereAttc);
	...
	public function Delete(){
        ...
        // if(!empty($data)){
	        ...
	        $sql = "SELECT COUNT(*) AS cnt FROM mgr.rl_prospect WHERE business_id ='".$business_id."'";
	       ...
	       $cons = $this->session->userdata('Tscons');
	        ...
	        $data = $this->m_wsbangun->deletedata_cons($cons,'rl_reserve_nup_hd',$where);
    ...
    public function SubmitStatus(){
       ...
       //          $profile_mail = $data[0]->email_profile;
	    	$cons = $this->session->userdata('Tscons');
	    		...
	    		$submit = $this->m_wsbangun->setData_by_query_cons($cons,$sql1);
	    		// var_dump($submit);
	    		// exit;
	...
	public function saveUpload()
    {
        if($_POST)
        {
            ...
            if(!empty($picture["name"]))
            {
                ...
                // $picname = str_replace(' ', '_', $files['userfile']['name']);
                $descs ="img/ROI/".$picname;
                $url=base_url().$descs;
                $sql = "UPDATE MGR.rl_nup_hd_attachment SET file_attachment='$picname', status_attach='1', audit_date='$today', file_url='$url'";
                $sql.= "WHERE rowID=$row";
