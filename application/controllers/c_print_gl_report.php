...
class c_print_gl_report extends Core_Controller
{
    ...
    public function index()
    {
    	...
    	$this->load_content_mgm('GLprint/index',$content);
    }
            ...
            if(!empty($parentID)) {
                ...
                foreach ($parentID as $dtParent) {
                  if($pro == $dtParent->row_id) {
                    ...
                    $comboParent[] = '<option '.$pilih.' value="'.$dtParent->report_id.'">'.$dtParent->report_id.' - '.$dtParent->descs.'</option>';
                }
...
public function zoom_start_end(){
     ...
     public function getTable()
    {
        ...
        $aColumns  = array('row_number', 'row_descs','space_descs','ref_no', 'module', 'field_id','type','start_exp','end_exp','percent_exp','seq_id');
        // $aColumns = array('entity_cd', 'entity_name');
        $sTable = 'mgr.v_cf_rpt_print';
        ...
        $param = " where module = 'GL' and report_id = '$report_id' ".$filter_search;
        // var_dump($param);
