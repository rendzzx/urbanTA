...
class C_Logindebtor extends Core_Controller
{
	function __construct()
    {
     ...
     public function index(){
        ...
        $this->load_content_top_menu('logindebtor/index',$data);
    ...
    public function getTable()
    {
       ...
       $entity = $this->session->userdata('Tsentity');
        ...
        $aColumns  = array('row_number','rowID','debtor_acct', 'name', 'hand_phone','email_addr');
        // $aColumns = array('entity_cd', 'entity_name');
        $sTable = 'mgr.v_logindebtor';
...
// var_dump($Search);
        ...
        $param =" Where entity_cd='$entity' AND project_no= '$project' AND [status]='A' $filter_search";
...
// var_dump($Search);
    ...
    private function EmailFormat($f_user_name,$f_project_name,$rt_url_link){
                      ...
                      $email .= '<div><h1>Yang Terhormat Bpk/Ibu '.$f_user_name.',</h1></div>'    ;
                      $email .= '<div> '     ;
   ...
   public function Approval(){
    ...
    $psn='';
        if($_POST){
            ...
            $table='v_logindebtor with(NOLOCK)';
            $where =array('debtor_acct'=>$debtor_acct,'entity_cd'=>$entity,'project_no'=>$project);
            ...
            $url= base_url('occupants/index/'.base64_encode($debtor_acct.'+'.$entity.'+'.$project.'+'.$datas[0]->email_addr));
            // var_dump($url);exit();
            
            $subject='Activation Login Occupants in IFCA System';
            $cc= '';$bcc='';
            $body=$this->EmailFormat($datas[0]->name,$ProjectData[0]->descs,$url);

            
            //None API Start
            if(!empty($datas) && !empty($datas[0]->email_addr)){
              $email =$datas[0]->email_addr;
              $mail = $this->_sendmail($email, $subject, $body);

              $msg = "Email sent";
              $psn ='Success';
              $aa = '';
            }else{
              $msg = "Don't Have Email";
              $psn ='Fail';
              $aa = '';
            }
            ...
            //API Start
            // $url = base_url('c_api_email/POST_EMAIL_API');
            // // var_dump($url); exit();trim(str)
            // // $url = 'http://localhost:2121/webproperty5.0/c_api_email/POST_EMAIL_API';
            // // $url = 'http://localhost:2121/emailAPI_/c_api_email/POST_EMAIL_API';
            // // $dataJson = "to=".$email."&subject=".$subject."&cc=".$cc."&bcc=".$bcc."&message=".$body;
            // $dataJson = "to=".$email."&subject=".$subject."&message=".$body;
            // $ch = curl_init();
            ...
            //     $sql = "mgr.xcf_debtor_approval_web '".$entity."', '".$project."','".$debtor_acct."', '".$url."' ,'debtor' ";
    ...
    public function Approval2(){
        if($_POST){
           ...
           // $sql ="SELECT  email_add,description FROM mgr.security_users where name "
            ...
            $subject='Activation Login Occupants in IFCA System';
            $cc= '';$bcc='';
            $body=$this->EmailFormat($datas[0]->description,$ProjectData[0]->descs,$url);
            
            
             if(!empty($datas) && !empty($datas[0]->email_add)){
    ...
    public function savestaff()
    {
        if($_POST)
        {
            ...
            if(!empty($models))
            {
                ...
                // var_dump($gID['GroupCd']);
                // exit(); 
              // var_dump(base64_encode($dt['name'].'_'.$dt['entity'].'_'.$dt['project']));
              // exit();
