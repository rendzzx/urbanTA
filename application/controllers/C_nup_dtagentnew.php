...
class C_nup_dtagentnew extends Core_Controller {
  ...
  //   	$this->m_wsbangun->setData_by_query($sql);
			...
			// ini yang diambil dari goto_form
			...
			// $sql.= "SELECT entity_cd, project_no, document_no, descs, STATUS, $seqno, '$user', '$dday' FROM mgr.rl_document_agent WHERE entity_cd='$entity' AND project_no='$project'";
	...
	public function list_dtNew($nup_no = null, $ops  = null, $rowID='',$status='')
	{
        ...
        $datanup = $this->m_wsbangun->getData_by_query($sql);
		...
		$databus = $this->m_wsbangun->getData_by_criteria($table,$where);
		$person='';
		if(! empty($databus)){
			$person = $databus[0];
		}
		...
		$sql = "SELECT MAX(property_cd) AS default_value FROM mgr.cf_property (NOLOCK) WHERE entity_cd = '$entity' and project_no = '$project' and default_value=1 and property_type = '$tipe'";
        ...
        $datalist2 = $this->m_wsbangun->getData_by_query($tabel2);
  //       $tabel2 = 'v_rl_reserve_nup_dt';
	 ...
	 // 		'nup_no'=>$nup_no);	 	
		...
		// var_dump($datalist2);
         ...
         // var_dump($sqlx);exit;
         $cntdata = $this->m_wsbangun->getData_by_query($sqlx);
		...
		// 	$Url = base_url("c_nup_unit/index/$RowId/$balance/$NupNO/$new/$pay/$add");
			
		// }
		// else{
		// var_dump($status_index);
			$Url = base_url("c_nup_landedAgentNew/indextipe/$NupNO/$a/$rowid_index/$tipe/$balance/$RowId/0/$new/$pay/$add"); 
		// }
		...
		$this->load_content_top_menu('unit/v_rl_nup_dtNew', $ContentAllData);
 ...
 //        echo json_encode($output);
	...
	public function getTableAttachDoc()
	{
        ...
        // $rResult = $DB2->get($sTable);
        // $rResult = $DB2->query($sql_data);
        $param =" Where nup_sequence_no=".$seqno." AND entity_cd='".$entity."' AND project_no= '".$project."' AND phase_cd='02' ".$filter_search;
        //' AND document_descs in ('KTP','NPWP') ".$filter_search;
        $field=" document_no,document_descs,file_attachment,rowID,nup_sequence_no ";
   		$rResult = $this->m_wsbangun->getlisttableattach($sTable,(int)($iDisplayStart),(int)($iDisplayLength),$SordField,$SortdOrder,$param,$field);
	...
	public function savenup2()
	{
		if($_POST){			
		  ...
		  //       $agent = $this->m_wsbangun->getData_by_criteria($table,$crit);
				...
				$update = $this->m_wsbangun->updateData('rl_reserve_nup',$data, $where);
				...
				if($update == 'OK')
                    	...
                    	$data = array(	                			
                    		...
                    		$update = $this->m_wsbangun->updateData('cf_business',$data, $where);
