...
class C_booking extends Core_Controller
{
    ...
    public function hitungKPR()
        ...
        $hasil = ($kredit*$bunga)*(1/(1-(1/(pow((1+$bunga),$waktu)))));
    ...
    public function index($property_type=null,$property_cd=null,$unit=null){
        ...
        if(!empty($property_type) && !empty($property_cd)){
            if($property_type=='A'){
                ...
                $this->load_content_top_menu('booking_steps/SB2UnitFloor',$data);
            }else if($property_type=='L'){
                $data = $this->data_unit_landed($property_cd,$property_type,$unit);
                $this->load_content_top_menu('booking_steps/SB2UnitLanded',$data);
            }
            
        }else{
            $this->load_content_top_menu('booking_steps/SB1',$data);  
        }
     ...
     public function indexNew(){
        ...
        $this->load_content_top_menu('booking_steps/index',$data);
    ...
    public function getTable()
    {
        ...
        $aColumns  = array('row_number','SP_No','NAME','lot_no','payment_plan','sell_price','status_desc','sales_date','Percentage','rowID');
...
// var_dump($Search);
    ...
    public function check_attachment($seqno='',$from=''){
        ...
        $sql = "SELECT count(*) as cnt , count(file_url) as counter FROM mgr.rl_sales_attachment(nolock) ";
        $sql.= "WHERE entity_cd='$entity' AND project_no='$project' AND sales_seq_no=$seqno ";
        $cons = $this->session->userdata('Tscons');
        $dtCnt = $this->m_wsbangun->getData_by_query2($sql);
        $cnt = $dtCnt[0]->counter;
        $ttlcnt = $dtCnt[0]->cnt;

        $sql2 = "SELECT isnull(max(payment_type),'') as typemax, isnull(max(payment_type_remarks),'') as remarksmax from mgr.rl_sales (NOLOCK)";
        $sql2 .= "WHERE entity_cd='$entity' AND project_no='$project' AND sales_seq_no=$seqno ";
    ...
    public function SubmitStatus(){
        if($_POST){
            ...
            $cons = $this->session->userdata('Tscons');
            $id = $this->input->post("id",true);//rowid rlsales
            // var_dump($id);exit;
            ...
            $where=array('entity_cd'=>$entity,
                        'project_no'=>$project,
                        'rowID'=>$id);
            $this->m_wsbangun->updateData_cons($cons,'rl_sales', $data, $where);

            $tbl = 'rl_nup_email';
            $cons = $this->session->userdata('Tscons');
            $dtEmail = $this->m_wsbangun->getData_cons($cons,$tbl);
            
            // $Email = 'deskaruwanda@gmail.com;';
            $Email = $dtEmail[0]->email;
            $Judul = 'Approval Booking Unit';
            
            $TisiEmail = 'v_rl_sales_booking';
            $kritIsiEmail = array('entity_cd' => $entity, 
                                    'project_no' => $project,
                                    'rowID' => $id);
            $QisiEmail = $this->m_wsbangun->getData_by_criteria_cons($cons,$TisiEmail, $kritIsiEmail);

            $data = array('sales_seqno' => $QisiEmail[0]->sales_seq_no,
                            'NAME' => $QisiEmail[0]->name,
                            'prod_descs' => $QisiEmail[0]->product_descs,
                            'booking_date'=>date_create($QisiEmail[0]->sales_date),
                            'booking_amt'=>$QisiEmail[0]->booking_fee_amt,
                            'lot_no' => $QisiEmail[0]->lot_no,
                            'payment_plan' => $QisiEmail[0]->payment_plan,
                            'sell_price'=>$QisiEmail[0]->sell_price,
                            'group_name' => $QisiEmail[0]->group_name,
                            'agent_name' => $QisiEmail[0]->agent_name,
                            'status'=>$QisiEmail[0]->status_desc);                 

            $body = $this->load->view('Email/EmailSubmitBooking', $data, true);
            // var_dump($Email);
    ...
    public function property_type_image($property_type=''){
        ...
        if(!empty($datalist2)){
            foreach ($datalist2 as $value) {
                ...
                if(!empty($value->map_picture)){                    
                    $a = '<a href="#" id="'.$value->property_cd.'" onclick="fn_click_image(\''.$value->property_cd.'\');"><center><img src="'.base_url('img/PlProject/'.$value->map_picture).'" style="width: 178px; height: 140px;" class="img-thumbnail"></center></a>';

                }else{
                    $a = '<a href="#" id="'.$value->property_cd.'" onclick="fn_click_image(\''.$value->property_cd.'\');" style="a:focus"><center><img src="'.base_url('img/PlProject/blankproject.png').'" style="width: 178px; height: 140px;" class="img-thumbnail"></center></a>';
                }
                $ListAllData .=$a;
                $ListAllData .='</div>';
        ...
        $this->load->view('booking_steps/property_type',$data);
    }
    public function property_image(){$entity = $this->session->userdata('Tsentity');
        $project = $this->session->userdata('Tsproject');
        $tabel2 = 'cf_property';
        $cons = $this->session->userdata('Tscons');
        $kriteria2 = array(
    ...
    public function SB3(){
            ...
            $this->load->view('booking_steps/SB3');
        }
    ...
    public function zoom_event()
    {
         ...
         $cons = $this->session->userdata('Tscons');
        $dtSpec = $this->m_wsbangun->getData_cons($cons,$table);
        if(!empty($dtSpec)) {
        ...
        $cons = $this->session->userdata('Tscons');
        $combo_event = $this->m_wsbangun->getCombo_cons($cons,$table,$crit,null,$dfMedia);
        return $combo_event;
    }
    public function zoom_payment_cd()
    {
       $table = 'rl_payment_plan_hd (nolock)';
        ...
        $cons = $this->session->userdata('Tscons');
        $combo_payment = $this->m_wsbangun->getCombo_cons($cons,$table,$crit,$where);
    ...
    public function zoom_nama($bussId)
    {
        ...
        $table = 'cf_business';
        $cons = $this->session->userdata('Tscons');
        $dtBuss = $this->m_wsbangun->getData_cons($cons,$table);
        if(!empty($dtBuss)) {
    ...
    public function finish(){
        ...
        $data=array('product'=>$dtProduct,
                    'project'=>$project,
                    'projectName'=>$projectName);
        $this->load_content_top_menu('booking_steps/SB5',$data);  
    }
    ...
    // var_dump($business_id);      
        ...
        $cons = $this->session->userdata('Tscons');
        $religion = $this->m_wsbangun->getCombo_cons($cons,$table,$crit);
        $crit = array('nationality_cd','descs');
        $nationality = $this->m_wsbangun->getCombo_cons('cf_nationality',$cons,$crit);
        ...
        $this->load_content_top_menu('booking_steps/SB3',$data);  
    }   
    public function add_payment($business_id=null,$property_type=null){
        ...
        $cons = $this->session->userdata('Tscons');
        $datalist = $this->m_wsbangun->getData_by_criteria_cons('cf_business',$cons,$where);  
        ...
        $where= array('entity_cd'=>$entity,
                      'business_id'=>$business_id);
        $cons = $this->session->userdata('Tscons');
        $datalist2 = $this->m_wsbangun->getData_by_criteria_cons('rl_sales',$cons,$where);      
        $ListAllData='';
        if(!empty($datalist2)){
            foreach ($datalist2 as $value) {
        ...
        $data=array('list_rl_sales'=>$ListAllData,
                    ...
                    'unit_book'=>$unit_book);
        $this->load_content_top_menu('booking_steps/SB4',$data);  
    }
    function clear_unit(){
        $entity = $this->session->userdata('Tsentity');
            $project = $this->session->userdata('Tsproject');  
    ...
    function goto_landed(){
        ...
        $this->load->view('booking_steps/property_landed',$data);
    ...
    public function showlanddt($lotno = null)
    {
        ...
        $this->load->view('booking_steps/infolanded',$content);
    }
    ...
    public function data_unit_landdt($rowid,$property_cd,$unit_book=null){
            ...
            $cons = $this->session->userdata('Tscons');
            $data = $this->m_wsbangun->getData_by_criteria_cons('cf_property_dtl',$cons, $where);
            ...
            $this->load_content_top_menu('booking_steps/SB2UnitLanddt',$ContentAllData);
            // $this->load_content_top_menu('booking_steps/SB2UnitLanddt',$ContentAllData);
    }
    ...
    public function to_unit($product,$property_cd){
        ...
        $where =array('entity_cd'=>$entity,
        			...
        			'property_cd'=>$property_cd);
        $cons = $this->session->userdata('Tscons');
        $data = $this->m_wsbangun->getData_by_criteria_cons('cf_property (NOLOCK)',$where);
        $property_type = $data[0]->property_type;
        if($property_type=='A'){
        	$this->load->view('booking_steps/SB2',$data);
        }else{
        	$this->load->view('booking_steps/SB2',$data);
        }
    ...
    public function zoom_property_type(){
    	$product_cd = $this->input->post('product_cd', TRUE);
            ...
            $cons = $this->session->userdata('Tscons');
            $cbProp = $this->m_wsbangun->getCombo_cons($cons,$table, $obj, $where);

        	// $data_project = $this->m_wsbangun->getData_by_criteria("cf_property",$where);    
        	echo $cbProp;
        }
    public function go_toPDF(){
    	$this->load->view('booking_steps/viewer');
    }
    public function datatable($property_cd='',$lot_no=null){
           ...
           /* var_dump($kriteria);*/
           $cons = $this->session->userdata('Tscons');
           $AllData = $this->m_wsbangun->getData_by_query_cons($sql);
        ...
        // var_dump($NupNo);
                ...
                $this->load->view('booking_steps/infoaptunit',$content);
            }
...
// var_dump($list_bf_price);exit();
				...
				$crit = array('userid'=>$webuser,
					'entity_cd'=>$entity);
                $cons = $this->session->userdata('Tscons');
				$dtAgent = $this->m_wsbangun->getData_by_criteria_cons($cons,$table, $crit);
				if(!empty($dtAgent)) {
				...
				$where = array('entity_cd'=>$entity,
								...
								'debtor_acct'=>$debtor);
                $cons = $this->session->userdata('Tscons');
				$cnt = $this->m_wsbangun->getCount_by_criteria_cons('rl_sales_payment',$cons,$where);
				if($cnt==0){
    ...
    public function discount_cd($payment_cd=null)
    {
        ...
        $where = array('payment_status' => 'A', 
                        'payment_cd'=>$payment_cd);
        $cons = $this->session->userdata('Tscons');
        $data = $this->m_wsbangun->getData_by_criteria_cons("rl_payment_plan_hd (NOLOCK) ",$cons,$where);
        // $data=$data->result();
