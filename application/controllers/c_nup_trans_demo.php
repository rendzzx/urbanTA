...
class C_Nup_trans_demo extends Core_Controller {
	...
	public function mainPage(){
        ...
        $cbnationality = $this->m_wsbangun->getCombo($table,$crit);


        $table = 'v_cf_agent_dt (nolock)';
        $crit = array('agent_cd', 'agent_name');
        $cbagent = $this->m_wsbangun->getCombo($table,$crit);
        ...
        if($_SERVER['SERVER_ADDR']=="192.168.0.25") { //bang pau
            $sitekey = '6Ld0kSMUAAAAAC0e0KLE3vwqFvb4_GZfH5XK7nHo';
            $secret='6Ld0kSMUAAAAAM3_DmYXT7FEkvKVPR-n_huAxM09';
        }
        ...
        if($_SERVER['SERVER_ADDR']=="localhost") { //demo
            $sitekey = '6Ld5RyQUAAAAAJX0oMVxXEoB2lgJeZuJ-4Aj74Wj';
            $secret='6Ld5RyQUAAAAACldDpSTOg-yTu_nfkh_AUsgDB8A';
        }
        ...
        // $this->load_content_top_menu('nup/v_rl_nupTrans',$content);

		$this->load->view('nup_demo/index_new', $content);
	}
    ...
    public function load_captcha(){
        ...
        $cp = $this->setCaptcha();
        $this->load->view('nup_demo/captcha',$cp);
    }
	...
	public function index()
    {
    	...
    	$this->load_content_top_menu('nup/NupIndexTrans',$ContentAllData);
    }
  ...
  //       $cnt = $this->check_attachment($seqno,'IN');
	...
	public function showinfo($status = null)
	{
		...
		$sql = "select nup_type_info, nup_loc_info from mgr.rl_nup_spec where entity_cd ='2101' and project_no = '210101'";
		...
		$content = array('info' => $info,
						'loc_info'=> $loc_info,
						'status' => $status);
        $this->load->view('nup_demo/nuptypeinfo',$content);
	}
	public function showformpayment(){
		...
		$this->load->view('nup_demo/formpayment',$content);
	}
	...
	public function zoom_nuptype(){
      ...
      public function zoom_property(){
        ...
        $sql = "select  product_type, descs from mgr.pm_product_dtl where entity_cd = '2101' and project_no = '210101' and status = 'Y' and product_cd='$product' ";
        $rst = $this->m_wsbangun->getData_by_query($sql);
        //var_dump($rst);
  ...
  //       return $cnt;
	...
	public function savenup()
	{
		if($_POST)
		{
		   ...
		   //      	var_dump($rsvdate);	
            ...
            $agent_cd =  $this->input->post('agent', TRUE);
	        ...
	        if($status=='N'){
	        	...
	        	$data = array('entity_cd'=>$entity,
                    ...
                    //submit
                    $table = 'rl_reserve_nup';
                    $crit = array('business_id'=>$number,
                                'reserve_by'=>'WEB');
                    $reserve = $this->m_wsbangun->getData_by_criteria($table,$crit); 
                    $id = $reserve[0]->rowID; 
                    $Newstatus='S';
                    $OldStatus=$reserve[0]->STATUS;
    ...
    public function SubmitStatus(){
    	if($_POST){
	    	...
	    	// var_dump($Email);
	    	// if(filter_var($Email, FILTER_VALIDATE_EMAIL)){
	    		// $sql ='Select max(email_profile) AS email_profile from mgr.rl_nup_spec';
       //          $data = $this->m_wsbangun->getData_by_query($sql);                    
       //          $profile_mail = $data[0]->email_profile;
	    		// $sql = "mgr.x_send_mail_php '".$profile_mail."', '".$Email."', '".$Judul."', '".$body."', '' ";
	    		// // var_dump($sql);
