...
class C_nup_dt extends Core_Controller {
  ...
  //   	$this->m_wsbangun->setData_by_query($sql);
		...
		if($tipe == 'A'){
			...
			//Apartement like Landed with mapster
			// $Url = base_url("C_nup_unitApartement/index/$RowId/$balance/$NupNO/$new/$pay/$add");
			
			//old version 13-03-2017
			...
			$Url = base_url("c_nup_landed/indextipe/$NupNO/$a/$rowid_index/$status_index/$balance/$RowId/0/$new/$pay/$add"); 
		}
// var_dump($url);
		if ($b == 0){
		...
		$this->load_content_top_menu('booking/v_rl_nup_dtNew', $ContentAllData);
