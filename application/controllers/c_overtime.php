...
class c_overtime extends Core_Controller
{
    ...
    public function zoom_zone_type(){
        ...
        //         if($over_cd == $result->over_cd){
        //           $pilih='selected';
        //         } else{
        //           $pilih='';
        //         }
        //         $combo[] = '<option '.$pilih.' value="'.trim($result->zone_cd).'" data-rate="'.$result->rate.'">'.$result->descs.'</option>';
        //     }
           echo json_encode($data);
      }
  ...
  //     }
    ...
    public function save(){
            ...
            if ($_POST) 
            {
                ...
                $trx_amt = $this->input->post('trx_amt',TRUE);
    ...
    public function getTable()
    {
        ...
        $param = " where ot_id > 0 and status ='N' ".$filter_search;
        // var_dump($param);
    ...
    public function add($date=''){
        ...
        if( $nameOfDay=='SAT' || $nameOfDay=='SUN'){
          //weekday
          // var_dump('a');
          $sql = "SELECT  end_time from mgr.cf_workhour(nolock) where entity_cd='$entity' and day_type='E'";
          ...
          // var_dump('b');
          //weekend
          $sql = "SELECT  end_time from mgr.cf_workhour(nolock) where entity_cd='$entity' and day_type='D'";
          $dt = $this->m_wsbangun->getData_by_query_cons($cons,$sql);
          if(!empty($dt)){
            $endhour_OT = date('g A', strtotime(substr ($dt[0]->end_time,11)));
            $start_overtime = date('H:i', strtotime(substr ($dt[0]->end_time,11)));
...
// exit();
    ...
    public function zoom_over(){
        ...
        $sql = "SELECT * from mgr.ot_type(nolock) where entity_cd='$entity' and project_no='$project'";
        $proDescs = $this->m_wsbangun->getData_by_query_cons($cons,$sql);
