...
class C_nup_landed_cfld extends Core_Controller
{
    ...
    public function index($unit=null){       
        ...
        // $descs_loop = $this->session->userdata('descs_loop');
        ...
        $sql = "SELECT property_cd AS default_value, property_type FROM mgr.cf_property (NOLOCK) WHERE entity_cd = '$entity' and project_no = '$project' and default_value=1 and property_type = 'L'";
        $defaulValue = $this->m_wsbangun->getData_by_query_cons($cons,$sql);
        $property_cd = empty($defaulValue)? '': $defaulValue[0]->default_value;
        $property_type = $defaulValue[0]->property_type;        
     ...
     //    $project = $this->session->userdata('Tsproject');
        ...
        $this->load_content_top_menu('booking_cfld/UnitLandedCfld',$data);
  ...
  //       }
    ...
    public function indexPrior($unit=null){       
        ...
        $data = $this->data_unit_landed($property_cd,$property_type,$unit);
        $this->load_content_top_menu('booking_cfld/UnitLandedCfldPrior',$data);

       
        
    }
    public function indexEdit($statusEdit = null, $Type = null){       
        ...
        if(empty($unit_loop)){
            $data = $this->data_unit_landed_edit_null($property_cd,$property_type,$statusEdit,$unit_loop,$headerid,$Type);
        } else {
            $data = $this->data_unit_landed_edit($property_cd,$property_type,$statusEdit,$unit_loop,$headerid,$Type);
        }
        
        $this->load_content_top_menu('booking_cfld/UnitLandedEditCfld',$data);
...
// var_dump($Search);
        ...
        if(!empty($datalist2)){
            foreach ($datalist2 as $value) {
                ...
                if(!empty($value->map_picture)){                    
                    ...
                    $a = '<a href="#" id="'.$value->property_cd.'" onclick="fn_click_image(\''.$value->property_cd.'\',\''.$value->property_type.'\');" style="a:focus"><center><img src="'.base_url('img/PlProject/blankproject.jpg').'" style="width: 178px; height: 140px;" class="img-thumbnail"></center></a>';
                }
                $ListAllData .=$a;
                $ListAllData .='</div>';
    ...
    public function property_image(){
        ...
        if(!empty($datalist2)){
            foreach ($datalist2 as $value) {
                ...
                $ListAllData .='</div>';
                $ListAllData .='<div class="product-desc">'; 
                $ListAllData .='<a href="#" id="'.$value->property_cd.'" onclick="fn_click_image(\''.$value->property_cd.'\',\''.$value->property_type.'\');" class="product-name">' .$value->descs. '&nbsp; <i class="fa fa-arrow-circle-right"></i><br><div style="font-size:12.5px;">'.$dtproduct[0]->descs.'</div></a>';
    ...
    public function data_unit_landed_edit_null($property_cd,$property_type,$statusEdit=null,$unit=null,$headerid=null,$Type=null){
            // var_dump('headerid'.' - '.$headerid);
            $cons = $this->session->userdata('Tscons');
            $entity = $this->session->userdata('Tsentity');
            $project = $this->session->userdata('Tsproject');
            $projectName = $this->session->userdata('Tsprojectname');
            ...
            // $dataDesc = explode(',', $unit);
            // $lotNo_string = "'" . implode("','",$dataDesc) . "'";
            // $descs = '';

            // foreach ($dataDesc as $value) {
            //     // var_dump($value);
            //     $sql = "SELECT descs FROM mgr.pm_lot_web WHERE entity_cd = '$entity' AND project_no = '$project' AND lot_no ='$value' ";
            //     $query = $this->m_wsbangun->getData_by_query($sql);
            //     // $descs = "'" .implode("','",$query[0]->descs)."'";
            //     $descs .= $query[0]->descs.",";
            // }

            // $b = strrpos($descs, ",");  
            // $descs =substr($descs, 0,$b);
            
            // $this->session->set_userdata('Tslotdescs', $descs);
            ...
            $sql = "SELECT project_no, property_cd, line_no, descs ,map_picture, rowID, coord = ISNULL(coord,''), coord_status = ISNULL(coord_status,0) from mgr.cf_property_dtl where property_cd = '".$property_cd."' and coord is not null and coord_status=1";
            ...
            if(!empty($query)){
                foreach ($query as $value) {
                    $areadata[] = '<area alt="" data-status="'.$status.'" title="" onclick="openPage(\''.$value->rowID.'\',\''.$property_cd.'\',\''.$statusEdit.'\',\''.$Type.'\',\''.$unit.'\')" href="#" shape="poly"  unit="'.$value->rowID.'" full="Unit '.$value->descs.'" coords="'.$value->coord.'">';
                    
                    if($value->coord_status ==0){
    ...
    public function data_unit_landed_edit($property_cd,$property_type,$statusEdit=null,$unit=null,$headerid=null,$Type=null){
            // var_dump('headerid'.' - '.$headerid);
            $cons = $this->session->userdata('Tscons');
            $entity = $this->session->userdata('Tsentity');
            $project = $this->session->userdata('Tsproject');
    ...
    public function data_unit_landed($property_cd,$property_type,$unit=null){            
            ...
            $sql = "SELECT project_no, property_cd, line_no, descs ,map_picture, rowID, coord = ISNULL(coord,''), coord_status = ISNULL(coord_status,0) from mgr.cf_property_dtl (NOLOCK) where property_cd = '".$property_cd."' and coord is not null and coord_status=1";
            $query = $this->m_wsbangun->getData_by_query_cons($cons,$sql);
            // var_dump($sql);
            $areadata[]='';
            $keyarea='';
            if(!empty($query)){
                foreach ($query as $value) {
                    $areadata[] = '<area alt="" data-status="'.$status.'" title="" onclick="openPage(\''.$value->rowID.'\',\''.$property_cd.'\',\''.$unit.'\')" href="#" shape="poly"  unit="'.$value->rowID.'" full="Unit '.$value->descs.'" coords="'.$value->coord.'">';
                    
                    if($value->coord_status ==1){
                        $keyarea.='{ key: "'.$value->rowID.'", selected: false},';
                    }
    ...
    public function showlanddt($lotno = null)
    {
        ...
        {
            $img= '<div class="item active">';
            $img .=  '<img alt="image"  class="img-responsive" src="'.base_url('img/LotInfo/unavailable.jpg').'" id="pop" onclick="imgpop(\'unavailable.jpg\')">';
            $img .= '</div>';
    ...
    public function data_unit_landdt($rowid,$property_cd,$unit_book=null){
            ...
            $business_id = $this->session->userdata('business_id');
            // $lot_descs = $this->session->userdata('descs_loop');
            // var_dump($lot_descs);
            ...
            $sql = "SELECT project_no, property_cd, level_no, lot_no, descs, coord = ISNULL(coord,''), coord_status = ISNULL(coord_status,0), nup_counter ,status from mgr.pm_lot_web (NOLOCK) where  coord is not null and property_dtl_rowid = '$rowid'";
            $query = $this->m_wsbangun->getData_by_query($sql);        
