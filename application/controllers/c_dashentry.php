...
class C_dashentry extends Core_Controller
{
    ...
    public function index()
    {
        ...
        $user = $this->session->userdata('Tsusergroup');
    	...
    	$this->load_content_top_menu('dashentry/index',$content);
    }
...
//         $comboParentID = $this->m_wsbangun->getCombo($table, $object);
    ...
    public function addnew(){
        ...
        $MenuData = $this->m_wsbangun->getData_by_criteriapb($table,$where);   
       
        $content = array('menuData'=>$MenuData);

        $this->load->view('dashentry/add', $content);

    }
    public function getByID($MenuID='',$GroupCd=''){
        // if(empty($id)){
        //     $id=''
        // }
        $where=array('MenuID'=>$MenuID,'GroupCd'=>$GroupCd);
        $data = $this->m_wsbangun->getData_by_criteriapb('sysDashGroup',$where);
    ...
    public function Delete(){
        ...
        $data = $this->m_wsbangun->deletedataweb('sysMenuMobile',$where);
    ...
    public function getTable()
    {
        ...
        $aColumns  = array('OrderSeq','Title','URL');
        // $aColumns = array('entity_cd', 'entity_name');
        $sTable = 'mgr.v_sysDashGroup';
    ...
    public function save_menu(){
            ...
            if ($_POST) 
            {
                $MenuID = $this->input->post('txtMenuID', TRUE);
                $MenuID2 = $this->input->post('MenuID2', TRUE);
                $action = $this->input->post('action', TRUE);
                // var_dump($MenuID);
                $pict = $this->input->post('pict',TRUE);
                $user = $this->session->userdata('Tsusergroup');            
                ...
                $audit_user = $this->session->userdata('Tsuname');
                    ...
                    $criteria = array('MenuID' => $MenuID2, 'GroupCd' => $user);
                    // var_dump($criteria);
                    if($action=='edit') {
                        $data = $this->m_wsbangun->updateDataweb('sysDashGroup',$data, $criteria);
                        // var_dump($data);exit();
                        if($data !="OK"){
                            $msg= $data;
                            $st = 'Fail';
                     ...
                     //   $this->m_user_log->insert(add_user_log("newsfeed Name " . $newsfeed_name, $this->m_users->get_by_uname($this->session->userdata('uname')), $this->m_activities->get_by_title("ADD newsfeed")));
                        ...
                        $data = $this->m_wsbangun->insertDataweb('sysDashGroup',$data);
                        if($data !="OK"){
                            $msg= $data;
                            $st = 'Fail';
                        }else{
