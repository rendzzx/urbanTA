...
class C_Nup extends Core_Controller {
	...
	public function check_attachment($seqno='',$from=''){
		...
		$cons = $this->session->userdata('Tscons');
		$dtCnt = $this->m_wsbangun->getData_by_query2_cons($const,$sql);
		$cnt = $dtCnt[0]->counter;
		$ttlcnt = $dtCnt[0]->cnt;
  ...
  //       $cnt = $this->check_attachment($seqno,'IN');
	...
	public function show_edit_data($ID=''){
		...
		$where =array('entity_cd'=>$entity,
					...
					'rowID'=>$ID);
		$cons = $this->session->userdata('Tscons');
		$data = $this->m_wsbangun->getData_by_criteria_cons($cons,'v_nup_update',$where);
	...
	public function saveUpload()
    {
        if($_POST)
        {
            ...
            if(!empty($picture["name"]))
            {
                ...
                if($psn=="OK"){
                	$descs ="img/NUP/".$picname;
	                $url=base_url().$descs;
	                $sql = "UPDATE MGR.rl_nup_attachment SET file_attachment='$picname', status_attach='1', audit_date='$today', file_url='$url'";
	                $sql.= "WHERE rowID=$row";
                	...
                	$msg="Error while uploading the picture";
                	$psn="Failed";
                }
                // $picname = str_replace(' ', '_', $files['userfile']['name']);
 ...
 //            // $crit = array('rowID'=>$row);
	...
	public function getTable($product_cd='')
    {
       ...
       $entity = $this->session->userdata('Tsentity');
        ...
        //untuk PK diharap diletakan di awal array
        $aField = array('id', 'subject', 'content','status');
        $aColumns  = array('rowID','NAME', 'nup_no', 'Handphone', 'Email', 'nup_type','reserve_date','STATUS');
        // $aColumns = array('entity_cd', 'entity_name');
...
// var_dump($Search);
        ...
        $param =" Where (status = 'A' or status = 'V' or (status = 'S' and old_status = 'V')) AND entity_cd='".$entity."' AND project_no= '".$project."' AND project_no='".$product_cd."' ".$filter_search;
        $cons = $this->session->userdata('Tscons');
...
// var_dump($Search);
        ...
        $param =" Where (status not in ('A','V', 'S','C','E') or (status = 'S' and old_status in ('R','N'))) AND entity_cd='".$entity."' AND project_no= '".$project."' ".$filter_search;
        $cons = $this->session->userdata('Tscons');
        $rResult = $this->m_wsbangun->getlisttablenup_cons($cons,$sTable,(int)($iDisplayStart),(int)($iDisplayLength),$SordField,$SortdOrder,$param);
